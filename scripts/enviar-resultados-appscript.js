  ...
  function percorrer(suite) {
    (suite.tests || []).forEach((t) => {
      ...
      if (t.state === "failed") {
        ...
        falhas.push({
          nome: t.title,
          nomeCompleto: t.fullTitle || t.title,
          erro: t.err && t.err.message ? t.err.message : "Erro não especificado",
        });
      }
    });
...
function buscarUrlPrintFalha(nomeSpecArquivo, nomeCompletoTeste) {
  ...
  if (!fs.existsSync(pastaSpecLocal)) return null;
  ...
  return `${urlBase}/${encodeURIComponent(nomeSpecArquivo + ".cy.js")}/${encodeURIComponent(arquivoEscolhido)}`;
}
