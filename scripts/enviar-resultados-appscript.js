...
//   // se teve retry, pega sempre a tentativa mais alta (é a que reflete o estado final da falha)
//   const comAttempt = candidatos.map((a) => ({ nome: a, attempt: parseInt((a.match(/attempt (\d+)/) || [])[1] || "1", 10) })).sort((a, b) => b.attempt - a.attempt);
//   const arquivoEscolhido = comAttempt[0].nome;
//   console.log(`   ✅ arquivo escolhido: "${arquivoEscolhido}"`);

//   const urlBase = `https://${REPO_OWNER}.github.io/${REPO_NAME}/reports/${RUN_NUMBER}/01_e2e/screenshots`;
//   const urlFinal = `${urlBase}/${encodeURIComponent(nomeSpecArquivo + ".cy.js")}/${encodeURIComponent(arquivoEscolhido)}`;
//   console.log(`   ✅ URL montada: ${urlFinal}`);
//   return urlFinal;
// }
...
//       const urlPrint = buscarUrlPrintFalha(nomeSpecArquivo, f.nomeCompleto);
//       return {
//         nome_teste: `${marca} - ${f.nome}`,
//         mensagem_erro: f.erro,
//         url_print_gh_pages: urlPrint || "",
//       };
//     });
